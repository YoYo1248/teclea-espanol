...
try {
  ...
  const nonVerbsEndingLikeInfinitives = new Set(['ayer', 'mujer', 'alquiler', 'bienestar'])
  ...
  for (const lesson of lessons) {
    ...
    for (const word of lesson.words) {
      ...
      const looksLikeInfinitive = !target.includes(' ') && /^(?:ir|[\p{L}]+(?:ar|er|ir)(?:se)?)$/u.test(target)
  ...
  if (kindCounts.单词 !== 375 || kindCounts.短语 !== 88 || kindCounts.动词原形 !== 85) errors.push(`单词/短语/动词原形应为 375/88/85 张，实际为 ${kindCounts.单词}/${kindCounts.短语}/${kindCounts.动词原形}`)
