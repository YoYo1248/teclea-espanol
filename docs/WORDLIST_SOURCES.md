...
## 当前版本的真实状态（2026-08-18）

当前共有 548 张 A1–B2 练习卡：

- 48 张场景单词与短语卡；
- 300 张 A1–A2 高频基础词与短表达卡；
- 200 张 B1–B2 常用词与短语卡（B1、B2 各 100 张）。

按当前训练类型统计为 375 张单词、88 张短语和 85 张常用动词原形。A1–B2 所有孤立的原形目标都已从普通词汇中抽出，统一进入动词专项；词库校验同时拒绝跨课程同形重复。原先从 20 个动词展开的 340 张人称/时态词形已从课程和错题来源中移除：它们会虚增词量，也缺少判断时态所必需的语境。原始数据与生成脚本仍保留，未来只用于“短语境提示 + 输入一个目标变位词”的独立模块。

当前等级主线数量为：A1 词汇与短语 249 张 + 动词原形 33 张；A2 主线 59 + 动词 7；B1 主线 72 + 动词 28；B2 主线 83 + 动词 17。小组仍用于保存进度和间隔练习，但用户首先看到的是完整等级主线。
...
新增 B1–B2 词库按“高使用价值 + 覆盖官方主题和交际功能 + 适合逐字输入”筛选：工作与项目、租房与行政、健康、教育、信息技术、社会议题、环境以及论证衔接等。每个等级仍是 100 张；经过原形抽取后按 5–12 项的小组重新组织。它不是从 PCIC 大段复制的内容，也不宣称覆盖考试全部词汇。
